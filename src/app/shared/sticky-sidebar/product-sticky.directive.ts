...
export class ProductStickyDirective implements OnInit {
      ...
      if (y > startPosition) {
        ...
        if (y > stopPosition) {
          this._renderer.setStyle(this._element.nativeElement, 'top', '' + (stopPosition - y) + 'px');
            ...
            let fixedTop = headerHeight.getBoundingClientRect().height + 100;
            if(topPosition > 200){
                this._renderer.setStyle(this._element.nativeElement, 'top', '100px');
            } else {
                  this._renderer.setStyle(this._element.nativeElement, 'top', '100px');
            }
